...
namespace Zephyr
{
    ...
    public class HelpMenu : Microsoft.Xna.Framework.DrawableGameComponent
    {
        ...
        protected override void LoadContent()
        {
           ...
           controls = Game.Content.Load<Texture2D>(@"Images\controls");
            rocknroll = Game.Content.Load<Texture2D>(@"Images\rocknroll");
            helpback = Game.Content.Load<Texture2D>(@"Images\helpback");
...
new Vector2(300, 100), Color.Black, 0, new Vector2(0, 0),
1, SpriteEffects.None, 1);
                  spriteBatch.DrawString(text, "Press Escape or B to move to Main menu",
new Vector2(350, 200), Color.Black, 0, new Vector2(0, 0),
1, SpriteEffects.None, 1);
        }
            else if (number == 1)
                spriteBatch.Draw(keyidea, new Vector2(300, 0), Color.White);
            else if (number == 2)
                spriteBatch.Draw(rocknroll, new Vector2(300, 0), Color.White);
            else if (number == 3)
                spriteBatch.Draw(pushit, new Vector2(300, 0), Color.White);
            else if (number == 4)
                spriteBatch.Draw(controls, new Vector2(300, 0), Color.White);
            spriteBatch.End();
        }
    }
}