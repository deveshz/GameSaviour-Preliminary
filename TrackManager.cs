...
namespace Zephyr
{
    ...
    /// </summary>
    public class TrackManager : Microsoft.Xna.Framework.DrawableGameComponent
    {
        ...
        Vector2 carPosition = new Vector2(300, 150);
       ...
       Texture2D mdg11;
        ...
        int enemySpawnMaxMilliseconds = 3000;
        int enemyMinSpeed = 10;
        int enemyMaxSpeed = 20;
        int nextSpawnTime = 0;
         ...
         protected override void LoadContent()
        {
            ...
            track = Game.Content.Load<Texture2D>(@"Images/I");
       ...
       carHeight + 100, true, SurfaceFormat.Color,DepthFormat.Depth24Stencil8);
            ...
            mdg1 = Game.Content.Load<Texture2D>(@"Images/mdg1");
            mdg1rep = Game.Content.Load<Texture2D>(@"Images/mdg1rep");
           
            mdg11 = Game.Content.Load<Texture2D>(@"Images/mdg1");
            
            mdg12 = Game.Content.Load<Texture2D>(@"Images/mdg1");
   
            mdg13 = Game.Content.Load<Texture2D>(@"Images/mdg1");
           
            mdg14 = Game.Content.Load<Texture2D>(@"Images/mdg1");
            ...
            trackoverlay = Game.Content.Load<Texture2D>(@"Images/overlayI");
           text = Game.Content.Load<SpriteFont>(@"Fonts\Snap");
           report = Game.Content.Load<Texture2D>(@"Images/report");
            ...
            for (int i = 0; i < spriteList3.Count; ++i)
            {
                ...
                if (s3.collisionRect.Intersects(carRect))
                {
                    ...
                    if (pause == false)
                    {
                        health = health + 5;
                        power_rep += 1;
                        soundBank.PlayCue("powercollision");
                    }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Draw(mdg1, new Vector2(40, 430), Color.White);
            spriteBatch.Draw(mdg11, new Vector2(120, 90), Color.White);
            if (power && power1 && power3 && power4)
            {
                spriteBatch.Draw(mdg12, new Vector2(700, 90), Color.White);
                a = true;
            }
            else
                power2 = false;
            spriteBatch.Draw(mdg13, new Vector2(330, 300), Color.White);
            spriteBatch.Draw(mdg14, new Vector2(500, 430), Color.White);
            if (power && power1 && power2 && power3 && power4)
            {
                spriteBatch.Draw(mdg8, new Vector2(700,450), Color.White);
                b = true;
            }
            else
                power5 = false;
            ...
            if (power)
            {
                ...
                spriteBatch.Draw(mdg1rep, new Rectangle(40, 430, 50, 50), Color.White);
               
            }
            if (power1)
            {

                spriteBatch.Draw(mdg1rep, new Rectangle(120, 90, 50, 50), Color.White);
                
            }
            if (power2)
            {

                spriteBatch.Draw(mdg1rep, new Rectangle(700, 90, 50, 50), Color.White);
                spriteBatch.Draw(obstaclerep, obstacleRect, Color.White);
            }
            if (power3)
            {

                spriteBatch.Draw(mdg1rep, new Rectangle(330, 300, 50, 50), Color.White);
                
            }
            if (power4)
            {

                spriteBatch.Draw(mdg1rep, new Rectangle(500, 430, 50, 50), Color.White);
               
            }
            if (power5)
            {
                spriteBatch.Draw(mdg1rep,new Rectangle(700,450,50,50), Color.White);
   ...
   new Vector2(car.Width / 2, car.Height / 2), SpriteEffects.None, 0);
            ...
            foreach (Sprite s1 in spriteList1)
...
1, SpriteEffects.None, 1);
            spriteBatch.DrawString(text, "" + mdg1_rep + "/5",
...
1, SpriteEffects.None, 1);
            ...
            spriteBatch.Draw(mhealthbar, new Rectangle(1200 / 2 - mhealthbar.Width / 2, 10, mhealthbar.Width, 44), new Rectangle(0, 45, mhealthbar.Width, 44), Color.Gray);

            //Draw the current health level based on the current Health
            spriteBatch.Draw(mhealthbar, new Rectangle(1200 / 2 - mhealthbar.Width / 2, 10, (int)(mhealthbar.Width * ((double)health / 100)), 44), new Rectangle(0, 45, mhealthbar.Width, 44), Color.Yellow);

            //Draw the box around the health bar
            spriteBatch.Draw(mhealthbar, new Rectangle(1200 / 2 - mhealthbar.Width / 2, 10, mhealthbar.Width, 44), new Rectangle(0, 0, mhealthbar.Width, 44), Color.White);
            spriteBatch.DrawString(text, "Power " + health,
new Vector2(500, 13), Color.Black, 0, new Vector2(0, 0),
1, SpriteEffects.None, 1);
        ...
        private bool CollisionOccurred(int aMove)
        {
            ...
            if (carRect.Intersects(mdg12Rect)&& a)
            {
                power2 = true;
                mdg1_rep += 1;
                soundBank.PlayCue("snare");
       ...
       GraphicsDevice.SetRenderTarget(trackRenderRotated);
        ...
        private void SpawnEnemy()
        {
            ...
            spriteList.Add(
            new ChasingSprite1(Game.Content.Load<Texture2D>(@"Images\anger"),
            position+ new Vector2(-120,80), new Point(60, 60), 10, new Point(0, 0),
            new Point(1, 1), speed, this));
            spriteList2.Add(
           new ChasingSprite1(Game.Content.Load<Texture2D>(@"Images\hunger"),
